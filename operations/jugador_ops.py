...
def agregar_jugador(jugador: JugadorConId) -> JugadorConId:
    ...
    with open(PLAYERS_CSV, mode='a', newline='', encoding='utf-8') as archivo:
        ...
        escritor.writerow([
            jugador.id, jugador.nombre, jugador.numero, jugador.posicion,
            jugador.equipo, jugador.altura, jugador.edad, jugador.estado, jugador.eliminado
        ])
    return jugador
    ...
    for jugador in jugadores:
        if jugador.id == id_jugador:
            ...
            with open(PLAYERS_CSV, mode='w', newline='', encoding='utf-8') as archivo:
                ...
                for j in jugadores:
                    escritor.writerow([
                        j.id, j.nombre, j.numero, j.posicion, j.equipo,
                        j.altura, j.edad, j.estado, j.eliminado
                    ])
            return jugador
    return None
...
def eliminar_jugador(id_jugador: int) -> Optional[JugadorConId]:
    ...
    if jugador_a_eliminar:
        with open(JUGELIM_CSV, mode='a', newline='', encoding='utf-8') as archivo:
            escritor = csv.writer(archivo)
            escritor.writerow([
                jugador_a_eliminar.id, jugador_a_eliminar.nombre, jugador_a_eliminar.numero,
